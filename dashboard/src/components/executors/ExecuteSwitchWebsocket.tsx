...
import { BackgroundToggle, InputToggle, LabelToggle, SwitchToggle, Toggle } from "../ui/switch-toggle";

interface ExecuteSwitchWebsocketProps {
    configuration: IConfiguration,
    dataConfiguration: IDataConfiguration,
    onSuccess: () => void
}
export default function ExecuteSwitchWebsocket({ configuration, dataConfiguration, onSuccess }: ExecuteSwitchWebsocketProps) {
    const [manualClose, setManualClose] = useState(false);
    const [isChecked, setIsChecked] = useState(false);
    const [connectionAttempts, setConnectionttempts] = useState(0);
    const socketRef = useRef<WebSocket | null>(null);
    const [isStarting, setIsStarting] = useState(false);
    const [data, setData] = useState<ISwitchWebsocketConfiguration>(
        dataConfiguration?.switchWebsocketConfiguration ? {
            host: dataConfiguration.switchWebsocketConfiguration.host,
            id: dataConfiguration.switchWebsocketConfiguration.id,
            protocol: dataConfiguration.switchWebsocketConfiguration.protocol,
            dataConfigurationId: dataConfiguration.switchWebsocketConfiguration.dataConfigurationId,
            identifier: dataConfiguration.switchWebsocketConfiguration.identifier
        } : {
    ...
    const handleCheckboxChange = () => {
        ...
        const dados = {
            [data.identifier]: novoValor? "ON": "OFF"
        };
        sendMessage(dados);
    }
                ...
                <CardContent className="flex items-center justify-center">
                    <LabelToggle>
                        <SwitchToggle>
                            ...
                            />
                            <BackgroundToggle />
                            <Toggle className={` ${isChecked ? 'translate-x-full' : ''
                                }`}>
